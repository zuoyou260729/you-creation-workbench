...
(function () {
  'use strict';
  window.APP_VERSION = 'v55';   // 与 sw.js 的 CACHE 版本保持一致，用于同步弹窗显示
  ...
  // 注意：二级分类 id 由数组下标生成（`${pid}_sub_${idx}`），故「删除」某一项时
  // 用 null 占位（而非真正移除），以保持后续下标不变、避免已入库物品的分类串位；
  // getSystemSecondary() 会过滤掉 null 项。新增分类一律追加到数组末尾。
  const SYSTEM_SECONDARY = {
    ...
    sys_clothing: ['T恤','衬衫','毛衣','卫衣','外套','羽绒服','西装','裤子','牛仔裤','半身裙','连衣裙','内衣','袜子','运动鞋','靴子','拖鞋',null,'双肩包','手提包','钱包','帽子','围巾','手套','皮带','其他'],
  ...
  // 说明：本表是「名称→图标」注册表，可能保留已从 SYSTEM_SECONDARY 移除的名称
  // （如 加湿器/雨鞋），不影响显示；保留以便日后把该分类加回来时图标可直接复用。
  ...
  function getSystemSecondary(pid){
    return (SYSTEM_SECONDARY[pid]||[]).map((name,idx)=>({
      ...
      parentId: pid,
      system: true
    })).filter(c=>c.name);   // null 占位项（已删除的分类）不参与展示；下标保持不变
  }
